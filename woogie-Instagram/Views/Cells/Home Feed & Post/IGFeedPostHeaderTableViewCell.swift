...
class IGFeedPostHeaderTableViewCell: UITableViewCell {
    ...
    override func layoutSubviews() {
        ...
        profilePhotoImageView.frame = CGRect(x: 2, y: 2, width: size+8, height: size+8)
        profilePhotoImageView.layer.cornerRadius = size/2
        
        moreButton.frame = CGRect(x: contentView.width-size, y: 2, width: size, height: size)
        usernameLabel.frame = CGRect(x: profilePhotoImageView.right+10, y: 2, width: contentView.width-(size*2), height: contentView.height-4)
    }
